...
- DB tables created: `wp_niztech_youtube_playlists` and `wp_niztech_youtube_videos` (schema created in migrations).
...
- Admin:
	- Settings → Niztech YouTube (or `options-general.php?page=niztech-youtube-config`) — enter YouTube V3 API key and save.
	- Edit a post of type `video_series` (plugin adds a metabox):
		- `Youtube URL` — paste a full YouTube playlist or video URL.
		- `Type` — choose `Playlist` or `Single Video`.
...
- Front-end:
	- Use the shortcode to render visible videos:
```html
